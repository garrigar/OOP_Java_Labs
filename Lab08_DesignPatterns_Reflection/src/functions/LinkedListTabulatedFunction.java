...
public class LinkedListTabulatedFunction implements TabulatedFunction, Externalizable {
    ...
    public Iterator<FunctionPoint> iterator() {
        return new Iterator<>() {
            ...
            public boolean hasNext() {
                return (current != head.prev || virgin) && (current.next.item != null);
            }
            ...
            public FunctionPoint next() {
                if (hasNext()) {
                    ...
                    return new FunctionPoint(current.item);
                } else
    ...
    /**
     * Empty constructor
     * For correct work of Externalizable and clone()
     */
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.pointsCount; i++) {
            ...
            builder.append(cur.item.toString());
            cur = cur.next;
        }
        builder.append('}');
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof LinkedListTabulatedFunction){
            LinkedListTabulatedFunction lltfObj = (LinkedListTabulatedFunction) obj;
            if (lltfObj.pointsCount != this.pointsCount) return false;
            FunctionNode objNode = lltfObj.head.next;
            FunctionNode thisNode = this.head.next;
            for (int i = 0; i < this.pointsCount; ++i){
                if (!objNode.item.equals(thisNode.item)) return false;
            ...
            FunctionNode thisNode = this.head.next;
            for (int i = 0; i < this.pointsCount; ++i){
                if (!tfObj.getPoint(i).equals(thisNode.item)) return false;
                thisNode = thisNode.next;
            }
            return true;
        }
    ...
    @Override
    public int hashCode() {
        int result = Integer.hashCode(this.pointsCount);
        FunctionNode cur = this.head.next;
        for (int i = 0; i < this.pointsCount; ++i){
            result = 31 * result + cur.item.hashCode();
            cur = cur.next;
        }
        return result;
    }
    ...
    /*protected*/ public Object clone() throws CloneNotSupportedException {
        ...
        if (this.pointsCount < 2) throw new CloneNotSupportedException();
        result.listInit(this.head.next.item, this.head.next.next.item);
        FunctionNode thisNode = this.head.next.next.next;
        for (int i = 2; i < this.pointsCount; ++i){
            //result.addNodeToTail().item = new FunctionPoint(thisNode.item);
                result.pointsCount++;
                FunctionNode newNode = new FunctionNode(new FunctionPoint(thisNode.item), result.head.prev, result.head.next);
                result.head.next.prev = newNode;
                result.head.prev.next = newNode;
                result.head.prev = newNode;

            thisNode = thisNode.next;
        }
        result.cacheNode = result.head.next;
        return result;
    }
